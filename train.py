...
if __name__ == "__main__":
    ...
    parser.add_argument('--model-name', default=None, type=str, help='used model name')
    # Model
    ...
    tokenizer = dh.get_tokenizer(args.model_name)
    # load data
    ...
    train_data = dh.load(args.fea_type, args.train_path, args.train_set,
                         ...
                         vocab=vocab, tokenizer=tokenizer, max_history_length=args.max_history_length,
    ...
    # create_model
    model = BartForConditionalGeneration.from_pretrained(args.model_name, ft_sizes=feature_dims)
    ...
    model.cuda()
    criterion = LabelSmoothing(size=len(vocab), padding_idx=vocab['<pad>'], smoothing=0.1)
    criterion.cuda()
